...
#include "bat/ads/internal/filters/ads_history_conversion_confirmation_type_filter.h"
...
TEST_F(BatAdsHistoryConversionConfirmationTypeFilterTest,
  ...
  // Act
  const std::deque<AdHistory> filtered_history = filter_->Apply(history);
  ...
  EXPECT_TRUE(CompareDequesAsSets(expected_history, filtered_history));
}
