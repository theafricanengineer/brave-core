...
void AdsImpl::Initialize(
  ...
  VLOG(1, "Initializing ads");

  initialize_callback_ = callback;

  if (IsInitialized()) {
    VLOG(1, "Already initialized ads");
...
void AdsImpl::InitializeStep5(
  ...
  VLOG(1, "Successfully initialized ads");
...
void AdsImpl::Shutdown(
  ...
  if (!is_initialized_) {
    VLOG(0, "Shutdown failed as not initialized");
...
void AdsImpl::OnUserModelLoaded(
  ...
  if (result != SUCCESS) {
    LOG(ERROR, "Failed to load user model for " << language << " language");
    return;
  }

  VLOG(3, "Successfully loaded user model for " << language << " language");
...
void AdsImpl::InitializeUserModel(
  ...
  VLOG(1, "Initializing user model for " << language << " language");

  user_model_.reset(usermodel::UserModel::CreateInstance());
  user_model_->InitializePageClassifier(json);

  VLOG(1, "Initialized user model for " << language << " language");
}
...
void AdsImpl::OnIdle() {
  VLOG(1, "Browser state changed to idle");
}

void AdsImpl::OnUnIdle() {
  if (!IsInitialized()) {
    VLOG(0, "OnUnIdle failed as not initialized");
    return;
  }

  VLOG(1, "Browser state changed to unidle");
...
void AdsImpl::OnMediaPlaying(
  ...
  VLOG(2, "Started playing media for tab id " << tab_id);
...
void AdsImpl::OnMediaStopped(
  ...
  VLOG(2, "Stopped playing media for tab id " << tab_id);
...
void AdsImpl::OnAdNotificationEvent(
  ...
  if (!ad_notifications_->Get(uuid, &info)) {
    LOG(ERROR, "Failed to trigger ad event as ad notification was not "
        "found for uuid " << uuid);
...
void AdsImpl::OnTabUpdated(
  ...
  if (is_active) {
    VLOG(2, "Tab id " << tab_id << " is active");
    ...
    VLOG(3, "Tab id " << tab_id << " is inactive");
...
void AdsImpl::ChangeLocale(
  ...
  if (std::find(languages.begin(), languages.end(), language_code)
      == languages.end()) {
    VLOG(1, language_code << " user model not supported for page "
        "classification");
  ...
  VLOG(1, "Changed to " << language_code << " user model for page "
      "classification");
...
void AdsImpl::OnPageLoaded(
  ...
  if (!IsInitialized()) {
    VLOG(0, "Failed to classify page as not initialized");
    return;
  }
  ...
  if (SameSite(url, last_shown_ad_notification_.target_url)) {
    VLOG(1, "Visited URL matches the last shown ad notification");
    ...
    if (!SameSite(url, last_sustained_ad_notification_.target_url)) {
      ...
      if (sustain_ad_notification_interaction_timer_.IsRunning()) {
        VLOG(1, "Already sustaining ad notification interaction for "
            "visited URL");
      } else {
        VLOG(1, "Already sustained ad notification interaction for "
            "visited URL");
      }
    }
  ...
  if (!last_shown_ad_notification_.target_url.empty()) {
    VLOG(1, "Visited URL does not match the last shown ad notification");
  }

  if (!is_supported_url) {
    VLOG(1, "Unable to classify page as visited URL is not supported");
    return;
  }

  if (SearchProviders::IsSearchEngine(url)) {
    VLOG(1, "Unable to classify page as visited URL is a search engine");
    return;
  }
...
void AdsImpl::ExtractPurchaseIntentSignal(
  ...
  VLOG(1, "Extracting purchase intent signal from visited URL");
}
...
std::string AdsImpl::ClassifyPage(
  ...
  if (winning_category.empty()) {
    VLOG(1, "Unable to classify page as not enough content");
    return "";
  }
  ...
  VLOG(1, "Successfully classified page as " << winning_category
  ...
  for (const auto& page_score : page_score_history) {
    ...
    for (size_t i = 0; i < page_score.size(); i++) {
      ...
      if (client_->IsFilteredCategory(taxonomy)) {
        VLOG(1, taxonomy << " taxonomy has been excluded from the winner "
            "over time");
...
void AdsImpl::ServeAdNotificationFromCategories(
  ...
  if (categories.empty()) {
    VLOG(1, "No categories to serve targeted ads");
    ServeUntargetedAdNotification();
    return;
  }

  VLOG(1, "Serving ad from categories:");
...
void AdsImpl::OnServeAdNotificationFromCategories(
  ...
  VLOG(1, "No eligible ads found in categories:");
  for (const auto& category : categories) {
    VLOG(1, "  " << category);
  }
...
bool AdsImpl::ServeAdNotificationFromParentCategories(
  ...
  VLOG(1, "Serving ad from parent categories:");
  for (const auto& parent_category : parent_categories) {
    VLOG(1, "  " << parent_category);
  }
...
void AdsImpl::ServeUntargetedAdNotification() {
  VLOG(1, "Serving ad notification from untargeted category");
...
void AdsImpl::OnServeAdNotification(
  ...
  VLOG(1, "Found " << eligible_ads.size() << " eligible ads");
...
void AdsImpl::FailedToServeAdNotification(
  ...
  VLOG(1, "Notification not made: " << reason);
...
CreativeAdNotificationList AdsImpl::GetEligibleAds(
  ...
  for (const auto& ad : unseen_ads) {
    ...
    for (const auto& exclusion_rule : exclusion_rules) {
      ...
      VLOG(1, exclusion_rule->GetLastMessage());
      should_exclude = true;
    }
    ...
    if (client_->IsFilteredAd(ad.creative_set_id)) {
      VLOG(1, "creativeSetId " << ad.creative_set_id << " excluded "
...
CreativeAdNotificationList AdsImpl::GetUnseenAdsAndRoundRobinIfNeeded(
  ...
  if (ads_for_unseen_advertisers.empty()) {
    VLOG(1, "All advertisers have been shown, so round robin");
  ...
  if (unseen_ads.empty()) {
    VLOG(1, "All ads have been shown, so round robin");
...
bool AdsImpl::IsAdNotificationValid(  // TODO(tmancey)
  ...
  if (info.title.empty() ||
    ...
    VLOG(1, "Notification not made: Incomplete ad information\n"
...
bool AdsImpl::IsAllowedToServeAdNotifications() {
  ...
  for (const auto& permission_rule : permission_rules) {
    ...
    VLOG(1, permission_rule->GetLastMessage());
    is_allowed = false;
  }
...
void AdsImpl::StartDeliveringAdNotifications() {
  ...
  VLOG(1, "Deliver ad notification " << FriendlyDateAndTime(time));
}
...
void AdsImpl::SustainAdNotificationInteractionIfNeeded() {
  ...
  VLOG(1, "Sustained ad notification interaction");
