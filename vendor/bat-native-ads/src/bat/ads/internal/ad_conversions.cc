...
void AdConversions::Check(
  ...
  VLOG(1, "Checking URL for ad conversion");
...
void AdConversions::StartTimerIfReady() {
  ...
  if (queue_.empty()) {
    VLOG(1, "Ad conversion queue is empty");
    return;
  }
...
void AdConversions::OnGetAdConversions(
  ...
  if (result != SUCCESS) {
    VLOG(1, "No ad conversions found");
    return;
  }
  ...
  for (const auto& ad_conversion : new_ad_conversions) {
    for (const auto& ad : ads_history) {
      ...
      VLOG(1, "Ad conversion for creative set " <<
          ad_conversion.creative_set_id << " for "
...
void AdConversions::ProcessQueueItem(
  ...
  if (creative_set_id.empty() || creative_instance_id.empty()) {
    VLOG(0, "Failed to convert ad for creative instance id "
        << creative_instance_id << " with creative set id " << creative_set_id
            << " " << friendly_date_and_time);
  } else {
    VLOG(1, "Successfully converted ad for creative instance id "
        << creative_instance_id << " with creative set id " << creative_set_id
            << " " << friendly_date_and_time);
...
void AdConversions::StartTimer(
  ...
  VLOG(1, "Started ad conversion timer for creative instance id "
...
void AdConversions::SaveState() {
  ...
  VLOG(4, "Saving ad conversions state");
...
void AdConversions::OnStateSaved(const Result result) {
  if (result != SUCCESS) {
    LOG(ERROR, "Failed to save ad conversions state");
    return;
  }

  VLOG(4, "Successfully saved ad conversions state");
}
...
void AdConversions::OnStateLoaded(
  ...
  if (result != SUCCESS) {
    VLOG(1, "Initializing ad conversions state");

    queue_.clear();
    SaveState();
  } else {
    if (!FromJson(json)) {
      LOG(ERROR, "Failed to load ad conversions state");

      VLOG(9, "Failed to parse ad conversions state:\n" << json);

      callback_(FAILED);
      return;
    }

    VLOG(3, "Successfully loaded ad conversions state");
  }
