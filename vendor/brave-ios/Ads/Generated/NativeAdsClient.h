...
class NativeAdsClient : public ads::AdsClient {
 ...
 private:
  ...
  void Log(const char * file, const int line, const ads::LogSeverity severity, const std::string & message) const override;
  void VerboseLog(const char * file, const int line, const int verbose_level, const std::string & message) const override;
};
