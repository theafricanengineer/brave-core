...
void AdsClientMojoBridge::Log(
  ...
  ads_client_->Log(file.c_str(), line, (ads::LogSeverity)severity, message);
}

void AdsClientMojoBridge::VerboseLog(
    const std::string& file,
    const int32_t line,
    const int32_t verbose_level,
    const std::string& message) {
  ads_client_->VerboseLog(file.c_str(), line, verbose_level, message);
}
